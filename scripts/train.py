...
from core.nnx_pgame import Transition
from core.nnx_pgame import NNXPgameEmitter, NNXPgameConfig
from qdax.utils.metrics import default_qd_metrics
import orbax.checkpoint as ocp

from core.networks import Actor, logits_to_action
from core import orbit_wars_jax
...
class R2SyncThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                cmd = ["aws", "s3", "sync", self.local_dir, f"s3://{self.bucket_name}/v6/", "--endpoint-url", self.endpoint]
                result = subprocess.run(cmd, capture_output=True, text=True)
                if result.returncode == 0:
                    print(f"[{time.strftime('%H:%M:%S')}] Successfully synced checkpoints to Cloudflare R2!")
                else:
...
def get_scoring_fn(actor_graph):
    ...
    def run_1v1(policy_p, opp_p, random_key):
        ...
        def scan_step(carry, _):
            ...
            ships_p0 = jnp.where(ships_p0 < 1.0, 0.0, ships_p0)
            ships_p1 = jnp.where(ships_p1 < 1.0, 0.0, ships_p1)
    ...
    def run_ffa(policy_p, opp1_p, opp2_p, opp3_p, random_key):
        ...
        def scan_step(carry, _):
            ...
            ships_p0 = jnp.where(ships_p0 < 1.0, 0.0, ships_p0)
            ships_p1 = jnp.where(ships_p1 < 1.0, 0.0, ships_p1)
            ships_p2 = jnp.where(ships_p2 < 1.0, 0.0, ships_p2)
            ships_p3 = jnp.where(ships_p3 < 1.0, 0.0, ships_p3)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="OrbitWars MAP-Elites Training Pipeline")
    parser.add_argument("--hardware", type=str, choices=["cpu", "tpu", "blackwell"], default="cpu", help="Target hardware environment")
    args = parser.parse_args()
    
    print(f"Initializing RL Pipeline for hardware target: {args.hardware.upper()}...")
    from dotenv import load_dotenv; load_dotenv(); import os
    
    # Configure R2 Sync Interval (Firmly set to 5 minutes to prevent data loss on Colab disconnects)
    sync_interval = 300
    R2SyncThread("/tmp/checkpoints_v6", os.environ.get("R2_BUCKET_NAME", "orbit-wars-checkpoints"), sync_interval).start()
    ...
    from core.networks import DoubleCritic
    ...
    print(f"Configuring NNXPgameEmitter for {args.hardware.upper()}...")
    if args.hardware == "cpu":
        env_batch = 8
        pg_batch = 4
    elif args.hardware == "tpu":
        env_batch = 128
        pg_batch = 32
    else:
        env_batch = 2048
    ...
    dummy_actions = jnp.zeros((60, 68), dtype=jnp.float32)
    ...
    for i in range(start_gen, num_iterations):
        ...
        if args.hardware == "tpu":
            try:
                ...
                if total > 0:
                    print(f"  -> TPU HBM Usage: {used:.2f} GB / {total:.2f} GB ({used/total*100:.1f}%)")
            except Exception:
                pass
        else:
            try:
                ...
                if vram_info and 'not found' not in vram_info.lower() and 'failed' not in vram_info.lower():
                    used_mb, total_mb = vram_info.split(', ')
                    print(f"  -> GPU VRAM Usage: {used_mb} MB / {total_mb} MB ({int(used_mb)/int(total_mb)*100:.1f}%)")
            except Exception:
                pass
        
        if i % 10 == 0:
            save_path = f"/tmp/checkpoints_v6/qdax_rep_{i}"
