...
class NNXPgameEmitter(Emitter):
    ...
    def state_update(self, emitter_state, repertoire, genotypes, fitnesses, descriptors, extra_scores):
        ...
        # Python loop: 32 calls × 8 (train_only) = 256 total gradient steps.
        # Each _jit_train_step compiles once to a single-step XLA binary (much smaller than a loop).
        for _ in range(32):
            emitter_state = self._jit_train_step(emitter_state)
    ...
    @partial(jax.jit, static_argnames=("self",))
    def _jit_train_step(self, emitter_state):
        """Single TD3 step: critic update → actor update → polyak. No lax.cond policy-delay branch."""
        should_train = emitter_state.replay_buffer.current_size >= self._config.learning_starts

        def do_train(es):
            rb = es.replay_buffer
            c_params, tc_params = es.critic_params, es.target_critic_params
            a_opt, c_opt = es.actor_opt_state, es.critic_opt_state
            a_params, ta_params = es.pg_actor_params, es.target_actor_params
            rng, step = es.random_key, es.steps
            ...
            return NNXPgameEmitterState(
                replay_buffer=rb, critic_params=c_params, target_critic_params=tc_params,
        ...
        return jax.lax.cond(should_train, do_train, lambda es: es, emitter_state)
